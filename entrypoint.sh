...
then
        ...
        echo "Creating mplayer config on user environment..." && sudo -u tmpuser -i mplayer >/dev/null 2>&1 && \
        echo "Executing command as TMPUSER..." && sudo -u tmpuser -D ${PWD} --preserve-env=SAL_USE_VCLPLUGIN,JAVA_HOME,LD_LIBRARY_PATH,IPED_VERSION -i exec "$@" || \
